...
#include "wifi-mac-header.h"//Joao'
...
WifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiPhy")
    ...
    .AddAttribute ("TxAntennas",
                   ...
                   UintegerValue (1),
                   MakeUintegerAccessor (&WifiPhy::GetNumberOfTxAntennas,
                                         &WifiPhy::SetNumberOfTxAntennas), //Joao
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RxAntennas",
                   "The number of supported Rx antennas.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&WifiPhy::GetNumberOfRxAntennas, //Joao
                                         &WifiPhy::SetNumberOfRxAntennas),
                   MakeUintegerChecker<uint32_t> ())
    ...
    .AddAttribute ("IsAtimWindow", //Joao
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&WifiPhy::GetIsAtimWindow,
                                        &WifiPhy::SetIsAtimWindow),
                   MakeBooleanChecker ())
...
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("InitializeFrequencyChannelNumber");
...
void
WifiPhy::SetIsAtimWindow (bool enable)
{
  m_isAtimWindow = enable;
}

bool
WifiPhy::GetIsAtimWindow (void) const
{
...
WifiPhy::Configure80211b (void)
{
  ...
  m_deviceMcsSet.push_back (WifiPhy::GetVhtMcs8 ()); //Joao, put the VHT in the 80211b
...
{
  NS_LOG_FUNCTION (this << standard);
  //NS_LOG_UNCOND("Configure Channel for Standard");
...
WifiPhy::ConfigureStandard (WifiPhyStandard standard)
{
  ...
  m_standard = standard;
  m_isConstructed = true;
  //NS_LOG_UNCOND("ConfigureStandard");
...
void
WifiPhy::SetNumberOfTxAntennas (uint32_t tx) //Joao
{
  m_numberOfTxAntennas = tx;
}

void
WifiPhy::SetNumberOfRxAntennas (uint32_t rx) //Joao
{
  m_numberOfRxAntennas = rx;
}

void
WifiPhy::SetNumberOfCommunicationLink (uint32_t link) //Joao
{
  NS_LOG_UNCOND(m_numberOfLinks);
  m_numberOfLinks = m_numberOfLinks + link;
  NS_LOG_UNCOND(m_numberOfLinks);
}

uint32_t
WifiPhy::GetNumberOfCommunicationLink (void) const //Joao
{
  return m_numberOfLinks;
}

uint8_t
WifiPhy::GetChannelCode (void) const //Joao
{
  return m_channelCode;
}

bool
WifiPhy::GetChannelLock (void) const //Joao
{
  return m_channelLock;
}

void
WifiPhy::SetChannelLock (bool channellock)//Joao
{
  m_channelLock = channellock;
}

void
WifiPhy::SetChannelCode (uint8_t channelcode) //Joao
{
  m_channelCode = channelcode;
}

uint32_t
WifiPhy::GetNumberOfTxAntennas (void) const //Joao
{
  return m_numberOfTxAntennas;
}

uint32_t
WifiPhy::GetNumberOfRxAntennas (void) const //Joao
{
...
{
  //NS_LOG_UNCOND("SetChannelNumber");
  NS_LOG_FUNCTION (this << +nch);
  if (m_isConstructed == false)
    {
      //NS_LOG_UNCOND("isconstruted false");
  ...
  if (GetChannelNumber () == nch)
    {
     // NS_LOG_UNCOND("No channel change requested");
...
WifiPhy::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, MpduType mpdutype)
{
  ...
  /*WifiMacHeader hdr;
  newPacket->PeekHeader(hdr);
  NS_LOG_UNCOND("phy " << hdr.GetDuration());*/

  StartTx (newPacket, txVector, txDuration);

  if (GetChannelCode() == 0 && GetIsAtimWindow())
  {
    SetSleepMode();
    NS_LOG_UNCOND("SLEEP CUZ SOMEONE GOT MY CHANNEL AND THERE IS NOT ANYMORE");
  }
