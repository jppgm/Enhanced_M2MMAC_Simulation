...
WifiRemoteStationManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiRemoteStationManager")
    ...
    .AddAttribute ("IsLowLatency",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("IsAtimWindow", //Joao
                   "If true, we try to model the ATIM-Window behaviour",
                   BooleanValue (false),
                   MakeBooleanAccessor (&WifiRemoteStationManager::GetIsAtimWindow,
                                        &WifiRemoteStationManager::SetIsAtimWindow),
                   MakeBooleanChecker ())
    ...
    .AddTraceSource ("MacTxFinalDataFailed",
                     ...
                     "ns3::Mac48Address::TracedCallback")
    .AddTraceSource ("MacTxAtimFailed", //Joao
                     "The transmission of a Atim packet by the MAC layer has failed",
                     MakeTraceSourceAccessor (&WifiRemoteStationManager::m_macTxAtimFailed),
                     "ns3::Mac48Address::TracedCallback")
    .AddTraceSource ("MacTxFinalAtimFailed",
                     "The transmission of a Atim has exceeded the maximum number of attempts",
                     MakeTraceSourceAccessor (&WifiRemoteStationManager::m_macTxFinalAtimFailed),
                     "ns3::Mac48Address::TracedCallback") //Joao
  ;
  return tid;
}
...
void
WifiRemoteStationManager::SetIsAtimWindow (bool enable) //Joao
{
  m_isAtimWindow = enable;
}

bool
WifiRemoteStationManager::GetIsAtimWindow (void) const //Joao
{
  return m_isAtimWindow;
}
...
uint32_t
WifiRemoteStationManager::GetMaxSsrc (void) const //Joao
{
  return m_maxSsrc;
}
...
void
WifiRemoteStationManager::ReportAtimFailed (Mac48Address address, const WifiMacHeader *header) //Joao
{
  //NS_LOG_UNCOND("WifiRemoteStationManager: ReportAtimFailed");
  NS_LOG_FUNCTION (this << address << *header);
  NS_ASSERT (!address.IsGroup ());
  WifiRemoteStation *station = Lookup (address, header);
  station->m_ssrc++;
 // NS_LOG_UNCOND("numero de tentativas :" << station->m_ssrc);
  m_macTxAtimFailed (address);
  DoReportAtimFailed (station);
}

void
WifiRemoteStationManager::DoReportAtimFailed (WifiRemoteStation *station) //joao
{
...
void
WifiRemoteStationManager::ReportAtimOk (Mac48Address address, const WifiMacHeader *header,
                                       double AtimAckSnr, WifiMode AtimAckMode, double AtimSnr) //Joao
{
  NS_LOG_FUNCTION (this << address << *header << AtimAckSnr << AtimAckMode << AtimSnr);
  NS_ASSERT (!address.IsGroup ());
  WifiRemoteStation *station = Lookup (address, header);
  station->m_state->m_info.NotifyTxSuccess (station->m_ssrc);
  station->m_ssrc = 0;
  DoReportAtimOk (station, AtimAckSnr, AtimAckMode, AtimSnr);
}

void
WifiRemoteStationManager::DoReportAtimOk (WifiRemoteStation *st,
                                        double AtimAckSnr, WifiMode AtimAckMode, double AtimSnr) //Joao
{
  NS_LOG_FUNCTION (this << st << AtimAckSnr << AtimAckMode << AtimSnr);
}
...
void
WifiRemoteStationManager::ReportFinalAtimFailed (Mac48Address address, const WifiMacHeader *header) //Joao
{
  NS_LOG_FUNCTION (this << address << *header);
  NS_ASSERT (!address.IsGroup ());
  WifiRemoteStation *station = Lookup (address, header);
  station->m_state->m_info.NotifyTxFailed ();
  station->m_ssrc = 0;
  m_macTxFinalAtimFailed (address);
  DoReportFinalAtimFailed (station);
}

void
WifiRemoteStationManager::DoReportFinalAtimFailed (WifiRemoteStation *station) //Joao
{
  NS_LOG_FUNCTION (this << station);
}
...
bool
WifiRemoteStationManager::NeedAtim (Mac48Address address, const WifiMacHeader *header, //Joao
                                   Ptr<const Packet> packet, WifiTxVector txVector)
{
  ...
  return DoNeedAtim (Lookup (address, header), packet, normally);
}
                                              ...
                                              Ptr<const Packet> packet)
{
  ...
  //NS_LOG_UNCOND("RtsCtsThreshold " << m_rtsCtsThreshold << "packetsize " << packet->GetSize());
...
{
  NS_ASSERT (!address.IsGroup ());
  //WifiMode ctsMode = GetControlAnswerMode (address, rtsMode); //joao
  WifiMode ctsMode = rtsMode;
  WifiTxVector v;
  v.SetMode (ctsMode);
...
bool
WifiRemoteStationManager::DoNeedAtim (WifiRemoteStation *station,
                                     Ptr<const Packet> packet, bool normally)
{
  return m_isAtimWindow;
}
...
bool
WifiRemoteStationManager::NeedAtimRetransmission (Mac48Address address, const WifiMacHeader *header, //Joao
                                                 Ptr<const Packet> packet)
{
  NS_LOG_FUNCTION (this << address << packet << *header);
  NS_ASSERT (!address.IsGroup ());
