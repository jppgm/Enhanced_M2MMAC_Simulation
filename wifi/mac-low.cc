...
MacLow::MacLow ()
  : m_normalAckTimeoutEvent (),
    ...
    m_atimAckTimeoutEvent (), //Joao
    m_sendAtimAckEvent (), //Joao
    m_ctsTimeoutEvent (),
    m_sendCtsEvent (),
    m_sendAckEvent (),
    m_sendDataEvent (),
    ...
    m_cfAckInfo (),
    m_atimnack_cond(false),
    m_comcond(false),
    last_channel(0),
    m_retrans_cond(false),
    m_waitingATIMACK(false)
{
  ...
  //m_channelrandom = CreateObject<UniformRandomVariable> (); //Joao
  //m_channelrandom->SetAttribute ("Min", DoubleValue (1));
  //m_channelrandom->SetAttribute ("Max", DoubleValue (25));
}
...
MacLow::DoDispose (void)
{
  ...
  m_atimAckTimeoutEvent.Cancel (); //Joao
  m_sendAtimAckEvent.Cancel ();//Joao
...
MacLow::CancelAllEvents (void)
{
  ...
  if (m_atimAckTimeoutEvent.IsRunning ()) //Joao
    {
      m_atimAckTimeoutEvent.Cancel ();
      oneRunning = true;
    }
  if (m_sendAtimAckEvent.IsRunning ()) //Joao
    {
      m_sendAtimAckEvent.Cancel ();
      oneRunning = true;
    }  
...
MacLow::SetPhy (const Ptr<WifiPhy> phy)
{
  ...
  m_phy->SetChannelCode(1);//(uint8_t)m_channelrandom->GetInteger()); //every node starts with the number 1
  m_phy->SetChannelLock(false);

  if (!m_stationManager->GetIsAtimWindow()) //in the com window
    {
      uint8_t adTo[6];
      m_self.CopyTo(adTo);
      NS_LOG_UNCOND(adTo[5]+0);

      if ((adTo[5]+1)%2) //the server and client applications differ by the address
      {
        //NS_LOG_UNCOND("MACLOW" << Simulator::Now ().GetSeconds ());
        Simulator::Schedule (Seconds(1), &MacLow::SendCtsToSelf, this); //servers send cts in the beginning of the window
      }
    }
}

void
MacLow::SetTxOp(Ptr<Txop> txop) //Joao
{
  m_constTxop = txop; 
} 
...
void
MacLow::SetAtimAckTimeout (Time atimAckTimeout)
{
  m_atimAckTimeout = atimAckTimeout;
}
...
bool
MacLow::GetComCond (void) const
{
  return m_comcond;
}

bool
MacLow::GetAtimNACKCond (void) const
{
  return m_atimnack_cond;
}
...
Time
MacLow::GetAtimAckTimeout (void) const
{
  return m_atimAckTimeout;
}
                           ...
                           Ptr<Txop> txop)
{
  ...
  if (NeedAtim ()) //Joao
    {
      m_txParams.EnableAtim ();
    }
  else
    {
      m_txParams.DisableAtim ();
    }
  //NS_LOG_UNCOND ("Mac-Low:Funcao StartTransmission(). Pergunta: MUST SEND ATIM");
  if (m_txParams.MustSendAtim () && !m_currentHdr.GetAddr1 ().IsGroup ())
    {
//         NS_LOG_UNCOND ("SIM, MUST SEND");
        //NS_LOG_UNCOND ("MacLow::StartTransmission::SendAtimForPacket");
        SendAtimForPacket ();       
    }
...
bool
MacLow::NeedAtim (void) //Joao
{
  WifiTxVector dataTxVector = GetDataTxVector (m_currentPacket, &m_currentHdr);
  return m_stationManager->NeedAtim (m_currentHdr.GetAddr1 (), &m_currentHdr,
...
uint8_t 
MacLow::SearchChannel(void)
{
  uint8_t i;
  for (i = 1; i < 26; i++)
  {
    if (m_channelCode_channels[i] == false)
    {
      return i;
    }
  }
...
//this fuction is used for when a station detect that another node has reserved the same channel
void
MacLow::UpdateChannel(void) 
{                           
  if (last_channel < 25) //considering 25 channels, if the last channel used is less than 25, it just updates
  {
    m_phy->SetChannelCode(last_channel + 1);
    NS_LOG_UNCOND (m_self << " channel: " << last_channel + 1);
  }
  else if (m_phy->GetChannelLock() == false) //if there is not any channel avaible and a channel has not been reserved yet, just sleep
  {
    NS_LOG_UNCOND ("GOING TO SLEEP NO MORE CHANNEL");
    m_phy->SetChannelCode(0);
    m_phy->SetSleepMode();
  }
  else //if a channel has been reserved and there is no more channel available, set the channel number to 0
  {
    m_phy->SetChannelCode(0);
  }
}
...
bool
MacLow::NoCommunicationFlow(void) //this fuction specifies if the station has reached the limit concerning the number of communication links it can deal with
{
  return (m_phy->GetNumberOfCommunicationLink() >= m_phy->GetNumberOfRxAntennas() - 1);
}

void
MacLow::StartATIMNACK(void)//prepare to send na ATIMNACK, if it is waiting for an ATIMNACK, wait until receiving it or the timeout is over
{
if (!m_atimAckTimeoutEvent.IsRunning())
 {
    m_atimnack_cond = true;
    m_constTxop->PrepareATIMNACK();
 }
 else
 {
    m_waitingATIMACK = true;
 }
}

//This function deals with most of the M2MMAC functionalities. The behavior of the simulation
//when receiving an ATIM, ATIMACK or ATIMNACK is implemented here
void
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe) //Joao
{
  ...
  /*if ((adTo[5]+0) == 2)
  {
    m_phy->SetChannelLock(true);
    m_phy->SetChannelCode(1);
  }*/
  //This it is used for when an ATIMNACK is received, some conditions here are used also for ATIM and ATIMACK
  //reception, so it will only be commented here to avoid repetition. 
  if(hdr.IsAtimNAck() && m_phy->GetChannelCode() != 0) //&& (adTo[5]+0) != 2) //if the node channel is the number 0, it is waiting
  {                                                    //to transmit an ATIMNACK to tell the other stations
    if (hdr.GetChannelCode() >= last_channel)          //with which it negotiated that there is not channel available.
    {
      last_channel = hdr.GetChannelCode(); //here we update the last channel used
    }

    if (m_phy->GetChannelLock() == false && last_channel >= m_phy->GetChannelCode()) //if the last used channel is greater than or equal to this station channel, just update it
    {
      UpdateChannel();
    }
    if (m_phy->GetChannelLock()==true && hdr.GetChannelCode() == m_phy->GetChannelCode())//if the channel has been reserved but another station has reserved the same
    {
      NS_LOG_UNCOND("SAMECHANNEL CANCEL " << m_self << " ");
      UpdateChannel();//update the channel
      StartATIMNACK();
    }

    hdr.GetAddr1().CopyTo(address_temp);
    //if the station has negotiated with the transmitter, update the channel code
    if (nodes_array[address_temp[5]+0] != 0 && hdr.GetChannelCode() != 0)
    {
        NS_LOG_UNCOND("NACKSHAKE from " << hdr.GetAddr1 () << " to " << m_self << " OK channel code " << (int)m_phy->GetChannelCode() << " channel code TX " << (int)hdr.GetChannelCode() << " ");
        nodes_array[address_temp[5]+0] = hdr.GetChannelCode();
    }
    else if (nodes_array[address_temp[5]+0] != 0 && hdr.GetChannelCode() == 0) //if it has negotiated and the new channel is 0, cancel the link and decrease the number of links 
    { 
      NS_LOG_UNCOND("DECREASING FLOW " << m_self);
      m_phy->SetNumberOfCommunicationLink(-1);
      if (m_phy->GetNumberOfCommunicationLink() == m_phy->GetNumberOfRxAntennas() - 2) //if the link limit had been reached, it may send ATIM again
      {
        m_constTxop->PrepareATIMNACK(); // //used just to trigger the ATIM transmission, it doesnt send any atimnack
      }
    }//if it is waiting for a ATIMACK from the transmitter, it interprets the ATIMNACK as so.
    else if (m_currentHdr.GetAddr1 () == hdr.GetAddr1() && m_atimAckTimeoutEvent.IsRunning() && hdr.GetChannelCode() != 0 && hdr.GetChannelCode_TX() == 31)
    {
       NS_LOG_UNCOND("HANDSHAKE from " << m_self << " to " <<  m_currentHdr.GetAddr1 () << " OK channel code " << (int)hdr.GetChannelCode() << " channel code TX " << (int)m_phy->GetChannelCode() << " ");
       nodes_array[address_temp[5]+0] = hdr.GetChannelCode();
       m_atimAckTimeoutEvent.Cancel ();
       NotifyAtimAckTimeoutResetNow ();
       m_currentTxop->GotAtimAck ();
       m_phy->SetNumberOfCommunicationLink(1);
       m_phy->SetChannelLock(true);
       SnrTag tag;
       ...
       m_stationManager->ReportAtimOk (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                      rxSnr, txVector.GetMode (), tag.Get ()); 
    }
  }
  //Function for the ATIM reception with a node with which it negotiated before
  if (hdr.IsAtim() && m_phy->GetChannelCode() != 0) //&& (adTo[5]+0) != 2)
  {
    hdr.GetAddr2().CopyTo(address_temp);
    if (nodes_array[address_temp[5]+0] != 0 && nodes_array[address_temp[5]+0] != hdr.GetChannelCode() && hdr.GetChannelCode() != 0)
    {
      NS_LOG_UNCOND("NACKSHAKE from " << hdr.GetAddr2 () << " to " << m_self << " OK channel code " << (int)m_phy->GetChannelCode() << " channel code TX " << (int)hdr.GetChannelCode() << " ");
      nodes_array[address_temp[5]+0] = hdr.GetChannelCode();
    }
    else if(nodes_array[address_temp[5]+0] != 0 && hdr.GetChannelCode() == 0)
    {
      NS_LOG_UNCOND("DECREASING FLOW " << m_self);
      m_phy->SetNumberOfCommunicationLink(-1);
      if (m_phy->GetNumberOfCommunicationLink() == m_phy->GetNumberOfRxAntennas() - 2)
      {
        m_constTxop->PrepareATIMNACK();  //used just to trigger the ATIM transmission, it doesnt send any atimnack
      }
    }
  ...
  if (hdr.IsAtim ()&& m_phy->GetChannelCode() != 0)// && (adTo[5]+0) != 2)//&& (adTo[5]+0) != 2)
  {

    if (hdr.GetChannelCode() >= last_channel)
    {
      last_channel = hdr.GetChannelCode();
    }

    if (m_phy->GetChannelLock() == false && last_channel >= m_phy->GetChannelCode())
    {
      UpdateChannel();
    }

    if (m_phy->GetChannelLock()==true && hdr.GetChannelCode() == m_phy->GetChannelCode() && hdr.GetChannelCode() != 0)
    {
      NS_LOG_UNCOND("SAMECHANNEL CANCEL " << m_self << " ");
      UpdateChannel();
      if (hdr.GetAddr1() == m_self && isPrevNavZero && !NoCommunicationFlow()) //if it is atim addressed to this station with the same channel code, then send an ATIMNACK as an ATIMACK 
      {
        m_stationManager->ReportRxOk (hdr.GetAddr2 (), &hdr,
                                            rxSnr, txVector.GetMode ());
        Simulator::Schedule (GetSifs (), &MacLow::SendAtimNACK, this, true);
        cond_ack = true;
        hdr.GetAddr2().CopyTo(address_temp);

        if (nodes_array[address_temp[5]+0] == 0) //if it is the first time it receives an ATIM (when sent an ATIMACK and was not decoded successfully)
        {
          m_constTxop->RemoveAtim(hdr); //remove the ATIM from the queue to stop double negotiations
          m_phy->SetNumberOfCommunicationLink(1);
        }
        nodes_array[address_temp[5] + 0] = hdr.GetChannelCode();

        if (NoCommunicationFlow() && m_stationManager->GetIsAtimWindow()) //if the link limit has been reached, cancel  timer
        {
          if (m_atimAckTimeoutEvent.IsRunning())
          {
            m_atimAckTimeoutEvent.Cancel();
  ...
  if (hdr.IsAtimAck () &&  m_phy->GetChannelCode() != 0)// && (adTo[5]+0) != 2)// && (adTo[5]+0) != 2)
  {
    uint8_t channel_available;
    channel_available = (hdr.GetChannelCode() > hdr.GetChannelCode_TX()) ? hdr.GetChannelCode() : hdr.GetChannelCode_TX();

    if (channel_available >= last_channel)
    {
      last_channel = channel_available;
    }

    if (hdr.GetAddr1 () != m_self && m_phy->GetChannelLock() == false && last_channel >= m_phy->GetChannelCode())
    {
      UpdateChannel();
    }

    if (hdr.GetAddr1 () != m_self && m_phy->GetChannelLock()==true && (hdr.GetChannelCode_TX() == m_phy->GetChannelCode() || hdr.GetChannelCode() == m_phy->GetChannelCode()))
    {
      NS_LOG_UNCOND("SAMECHANNEL CANCEL " << m_self << " ");
      UpdateChannel();
      StartATIMNACK();
  ...
  if(hdr.IsCts())//used in the communication window, to allow the stations to send data packet once they receive the CTS
  {
    m_comcond = true;
    m_constTxop->GotCts();
  }
//the next two functions are equivalent to the RTS and CTS functions. They just implement the normal handshake
if (hdr.IsAtim ())
  {
      //NS_LOG_UNCOND ("MacLow::ReceiveOk: IsAtim");
      if (ampduSubframe)
        {
          NS_FATAL_ERROR ("Received ATIM as part of an A-MPDU");
        }
      else
        {
          if (isPrevNavZero
              && hdr.GetAddr1 () == m_self && cond_ack == false && hdr.GetChannelCode() != 0 && !NoCommunicationFlow())
            {
              //NS_LOG_UNCOND (hdr.GetDuration());
              if(m_phy->GetChannelLock() == false)
              {
                m_phy->SetChannelLock(true); //to lock (reserve) the channel
              }

              NS_LOG_DEBUG ("rx ATIM from=" << hdr.GetAddr2 () << ", schedule ATIMACK");
              NS_ASSERT (m_sendAtimAckEvent.IsExpired ());
              m_stationManager->ReportRxOk (hdr.GetAddr2 (), &hdr,
                                            rxSnr, txVector.GetMode ());
              m_sendAtimAckEvent = Simulator::Schedule (GetSifs (),
                                                     &MacLow::SendAtimAckAfterAtim, this,
                                                     hdr.GetAddr2 (),
                                                     txVector,
                                                      rxSnr,
                                                      hdr.GetChannelCode());
              ...
              hdr.GetAddr2().CopyTo(address_temp);
              if (nodes_array[address_temp[5]+0] == 0){
                m_constTxop->RemoveAtim(hdr);
                m_phy->SetNumberOfCommunicationLink(1); //increasing the number of communication links on this node Joao
              }
              nodes_array[address_temp[5] + 0] = hdr.GetChannelCode();

              if (NoCommunicationFlow() && m_stationManager->GetIsAtimWindow())
              {
                if (m_atimAckTimeoutEvent.IsRunning())
                {
                  m_atimAckTimeoutEvent.Cancel();
            ...
            {
              NS_LOG_DEBUG ("rx ATIM from=" << hdr.GetAddr2 () << ", cannot schedule ATIMACK");
            }
        }
  } //Caso nao seja, veja se eh ATIMACK
  else if (hdr.IsAtimAck ()
           && hdr.GetAddr1 () == m_self
           && m_atimAckTimeoutEvent.IsRunning ()
           && m_currentPacket != 0
           && hdr.GetChannelCode() != 0
           && m_phy->GetChannelCode() != 0)
  {
      //NS_LOG_UNCOND ("MacLow::ReceiveOk: IsAtimAck ");
      if (ampduSubframe)
        {
          NS_FATAL_ERROR ("Received ATIMACK as part of an A-MPDU");
        }

      NS_LOG_DEBUG ("receive ATIMACK from=" << m_currentHdr.GetAddr1 ());
      ...
      m_currentHdr.GetAddr1().CopyTo(address_temp);
      nodes_array[address_temp[5] + 0] = hdr.GetChannelCode();
      m_phy->SetNumberOfCommunicationLink(1); //increasing the number of communication links on this node Joao
      SnrTag tag;
      packet->RemovePacketTag (tag);
      ...
      m_stationManager->ReportAtimOk (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                      rxSnr, txVector.GetMode (), tag.Get ()); 
      NS_LOG_UNCOND("HANDSHAKE from " << hdr.GetAddr1 () << " to " << m_currentHdr.GetAddr1 () << " OK channel code " << (int)hdr.GetChannelCode() << " channel code TX " << (int)hdr.GetChannelCode_TX() << " ");
      m_atimAckTimeoutEvent.Cancel ();
      NotifyAtimAckTimeoutResetNow ();
      ...
      //FlushAggregateQueue (); //????
      m_ampdu = false;
    // NS_LOG_UNCOND (m_phy->GetNumberOfCommunicationLink());

  }//Caso nao seja pode seguir
  //----------------------------------// 
  else if (hdr.IsRts ())
    {
      ...
      if (!m_txParams.HasNextPacket ())
        {
         ...
         NS_LOG_UNCOND ("DATATX from " << hdr.GetAddr1 () << " to " << m_currentHdr.GetAddr1 () << " OK ");
        }
      bool gotAck = false;
...
Time
MacLow::GetAtimAckDuration (Mac48Address to, WifiTxVector atimTxVector) const //Joao
{
  WifiTxVector atimAckTxVector = GetAtimAckTxVectorForAtim (to, atimTxVector.GetMode ());
  return GetAtimAckDuration (atimAckTxVector);
}

Time
MacLow::GetAtimAckDuration (WifiTxVector atimAckTxVector) const //Joao
{
  NS_ASSERT (atimAckTxVector.GetMode ().GetModulationClass () != WIFI_MOD_CLASS_HT); 
  return m_phy->CalculateTxDuration (GetAtimAckSize (), atimAckTxVector, m_phy->GetFrequency ());
}
...
WifiTxVector
MacLow::GetAtimTxVector (Ptr<const Packet> packet, const WifiMacHeader *hdr) const //Joao
{
  Mac48Address to = hdr->GetAddr1 ();
  return m_stationManager->GetRtsTxVector (to, hdr, packet);
}
...
WifiTxVector
MacLow::GetAtimAckTxVector (Mac48Address to, WifiMode atimTxMode) const //Joao
{
  return m_stationManager->GetCtsTxVector (to, atimTxMode);
}
...
WifiTxVector
MacLow::GetAtimAckTxVectorForAtim (Mac48Address to, WifiMode atimTxMode) const //Joao
{
  return GetAtimAckTxVector (to, atimTxMode);
}
...
void
MacLow::NotifyNav (Ptr<const Packet> packet, WifiMacHeader &hdr)
{
  Time duration = Seconds (0); //since the ATIM,ATIMNACK e ATIMACK have fixed length, we implement the NAV manually to reduce the collision probability
  if(hdr.IsAtim())
  {
    WifiTxVector atimTxVector = GetAtimTxVector (packet, &hdr);
    duration += GetSifs ();
    duration += GetAtimAckDuration (hdr.GetAddr1 (), atimTxVector);
    hdr.SetDuration(duration);
    duration = hdr.GetDuration();
    //NS_LOG_UNCOND(duration << "ATIM NAV");
  }
  else if (hdr.IsAtimAck() || hdr.IsAtimNAck())
  {
    WifiTxVector atimTxVector = GetAtimTxVector (packet, &hdr);
    duration += GetSifs ();
    duration += GetAtimAckDuration (hdr.GetAddr1 (), atimTxVector);
  ...
  if (hdr.GetRawDuration () > 32767 && (!hdr.IsAtim()) && (!hdr.IsAtimAck()))
    {
...
void
MacLow::NotifyAtimAckTimeoutStartNow  (Time duration) //Joao
{
  for (ChannelAccessManagersCI i = m_channelAccessManagers.begin (); i != m_channelAccessManagers.end (); i++)
    {
      (*i)->NotifyAtimAckTimeoutStartNow (duration);
    }
}

void
MacLow::NotifyAtimAckTimeoutResetNow () //Joao
{
  for (ChannelAccessManagersCI i = m_channelAccessManagers.begin (); i != m_channelAccessManagers.end (); i++)
    {
      (*i)->NotifyAtimAckTimeoutResetNow ();
    }
}

void
MacLow::ForwardDown (Ptr<const Packet> packet, const WifiMacHeader* hdr, WifiTxVector txVector) //Joao
{
  ...
  if (!m_ampdu || hdr->IsAck () || hdr->IsRts () || hdr->IsCts () || hdr->IsBlockAck () || hdr->IsMgt () || hdr->IsAtim () || hdr->IsAtimAck ())
    {
     ...
     // WifiMacHeader hdrtemp;
     // packet->PeekHeader(hdrtemp);
     // NS_LOG_UNCOND("mac2 " << hdrtemp.GetDuration());
...
void
MacLow::AtimAckTimeout (void) //Joao
{
  //NS_LOG_UNCOND("MacLow: AtimAckTimeout");
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("atim timeout");
  /// \todo should check that there was no rx start before now.
  /// we should restart a new cts timeout now until the expected
  /// end of rx if there was a rx start before now.
  m_stationManager->ReportAtimFailed (m_currentHdr.GetAddr1 (), &m_currentHdr);
  Ptr<Txop> txop = m_currentTxop;
  m_currentTxop = 0;
  m_ampdu = false;
  if (m_waitingATIMACK)
  ...
  m_retrans_cond = txop->MissedAtimAck ();
}
...
void
MacLow::SendAtimForPacket (void) //Joao
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("MacLow: SendAtimForPacket");
  /* send an ATIM for this packet. */
  WifiMacHeader atim;
  atim.SetType (WIFI_MAC_CTL_ATIM);
  atim.SetDsNotFrom ();
  atim.SetDsNotTo ();
  atim.SetNoRetry ();
  atim.SetNoMoreFragments ();
  atim.SetAddr1 (m_currentHdr.GetAddr1 ());
  atim.SetAddr2 (m_self);
  atim.SetChannelCode(m_phy->GetChannelCode());
  atim.SetChannelCode_TX(uint8_t(0));
  WifiTxVector atimTxVector = GetAtimTxVector (m_currentPacket, &m_currentHdr);
  //Time duration = Seconds (0);  

  //duration += GetSifs ();
  //duration += GetAtimAckDuration (m_currentHdr.GetAddr1 (), atimTxVector);
  /////////////////////////////////////////////////////////////////////////////
  //duration += GetSifs ();
  //NS_LOG_UNCOND (duration);
  //duration += GetAtimAckDuration (m_currentHdr.GetAddr1 (), atimTxVector);
...
/////////////////////////////////////////////////////////////////////////////////
  //atim.SetDuration (duration);
  //NS_LOG_UNCOND (atim.GetDuration());

  Time txDuration = m_phy->CalculateTxDuration (GetAtimSize (), atimTxVector, m_phy->GetFrequency ());
  Time timerDelay = txDuration + GetAtimAckTimeout ();
  NS_ASSERT (m_atimAckTimeoutEvent.IsExpired ());
  NotifyAtimAckTimeoutStartNow (timerDelay);
  m_atimAckTimeoutEvent = Simulator::Schedule (timerDelay, &MacLow::AtimAckTimeout, this);
  //NS_LOG_UNCOND(timerDelay << "ATIMACK TIMEOUT");
  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (atim);
  AddWifiMacTrailer (packet);

 //WifiMacHeader hdrtemp;
  //packet->PeekHeader(hdrtemp);
 // NS_LOG_UNCOND("mac " << hdrtemp.GetDuration());
 // NS_LOG_UNCOND ("MAC-LOW::SendAtimForPacket:: ForwardDown().");

  m_retrans_cond = false;
  //m_atimnack_cond = false;
  ForwardDown (packet, &atim, atimTxVector);
}
...
void
MacLow::SendAtimAckAfterAtim (Mac48Address source, WifiTxVector atimTxVector, double atimSnr, uint8_t channelcode_tx) //Joao
{
 // NS_LOG_UNCOND ("MacLow::SendAtimAckAfterAtim");
  NS_LOG_FUNCTION (this << source << atimTxVector.GetMode () << atimSnr);
  /* send a ATIMACK when you receive a ATIM
   * right after SIFS.
   */
  WifiTxVector atimAckTxVector = GetAtimAckTxVector (source, atimTxVector.GetMode ());
  WifiMacHeader atimAck;
  atimAck.SetType (WIFI_MAC_CTL_ATIMACK);
  atimAck.SetDsNotFrom ();
  atimAck.SetDsNotTo ();
  atimAck.SetNoMoreFragments ();
  atimAck.SetNoRetry ();
  atimAck.SetAddr1 (source);
  atimAck.SetChannelCode(m_phy->GetChannelCode());
  atimAck.SetChannelCode_TX(channelcode_tx);
  //Time duration = Seconds (0); 
  //NS_LOG_UNCOND("Duration Antes");
  //NS_LOG_UNCOND(duration);
  //duration -= GetAtimAckDuration (source, atimTxVector);
  //NS_LOG_UNCOND("Duration - getatimackduration");
  //NS_LOG_UNCOND(duration);
  //duration -= GetSifs ();
  //NS_LOG_UNCOND(duration);
  //NS_ASSERT (duration >= Seconds (0));

  //atimAck.SetDuration (duration); //???
  //NS_LOG_UNCOND(atimAck.GetDuration() << "ATIMACK");

  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (atimAck);
  AddWifiMacTrailer (packet);
  ...
  //CTS should always use non-HT PPDU (HT PPDU cases not supported yet) AND ATIM ALSO
  //NS_LOG_UNCOND ("MacLow::SendAtimAckAfterAtim:: Forward Down");
  ForwardDown (packet, &atimAck, atimAckTxVector);
}

void
MacLow::SendAtimNACK (bool isATIMACK) //Joao, send ATIMNACK
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("MacLow: SendAtimForPacket");
  /* send an ATIM for this packet. */
  WifiMacHeader atimnack;
  atimnack.SetType (WIFI_MAC_CTL_ATIMNACK);
  atimnack.SetDsNotFrom ();
  atimnack.SetDsNotTo ();
  atimnack.SetNoRetry ();
  atimnack.SetNoMoreFragments ();
  atimnack.SetAddr1 (m_self);
  atimnack.SetChannelCode(m_phy->GetChannelCode());
  if (isATIMACK == true)
  {
    atimnack.SetChannelCode_TX(uint8_t(31));
  }
  else
  {
    atimnack.SetChannelCode_TX(uint8_t(0));
  }
  WifiTxVector atimTxVector = GetAtimTxVector (m_currentPacket, &m_currentHdr);

  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (atimnack);
  AddWifiMacTrailer (packet);
  NS_LOG_UNCOND ("MacLow::SendAtimNack:: Forward Down");

  m_atimnack_cond = false;
  //m_phy->SetChannelLock(true);

  ForwardDown (packet, &atimnack, atimTxVector);
}
...
{
  //NS_LOG_UNCOND("DeaggregatingAMPDU");
  NS_LOG_FUNCTION (this);
  AmpduTag ampdu;
...
{
  //NS_LOG_UNCOND("Perfoming MSDU Aggregation");
