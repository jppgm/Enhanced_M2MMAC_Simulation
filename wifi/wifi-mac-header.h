...
enum WifiMacType
{
  ...
  WIFI_MAC_CTL_ATIMNACK,
  WIFI_MAC_CTL_ATIM, // Joao
  WIFI_MAC_CTL_ATIMACK,
  WIFI_MAC_CTL_RTS,
  WIFI_MAC_CTL_CTS,
  WIFI_MAC_CTL_ACK,
...
public:
  ...
  /**
   * Set the Reception Frequency Channel Code field with the given channel code
   *
   * \param channel the given channel code
   */
  void SetChannelCode(uint8_t channel); //Joao
  void SetChannelCode_TX(uint8_t channel); //Joao
  void SetNegotiations(uint8_t negotiations); //Joao
  /**
   * Set the Number of Data Packets field with the given number of data packets
   *
   * \param numberPackets the given number of data packets  
   */
  void SetNumberOfDataPackets(uint16_t numberPackets); //Joao
  /**
  ...
  /**
   * Return true if the header is a ATIM header.
   *
   * \return true if the header is a ATIM header, false otherwise
   */
   bool IsAtim (void) const; //Joao
  /**
   * Return true if the header is a ATIMACK header.
   *
   * \return true if the header is a ATIMACK header, false otherwise
   */
  bool IsAtimAck (void) const; //Joao 
  /**
   * Return true if the header is a ATIMNACK header.
   *
   * \return true if the header is a ATIMNACK header, false otherwise
   */
  bool IsAtimNAck (void) const; //Joao 
  /**
  ...
  /**
   * Return the channel code from the Reception Frequency Channel Code field of the ATIM header
   *
   * \return the reception frequency channel code field   
   */
  uint8_t GetChannelCode(void) const; //Joao
  uint8_t GetChannelCode_TX(void) const; //Joao
  uint8_t GetNegotiations(void) const; //Joao
  /**
   * Return the number of packets from the the Number of Data Packets field of the ATIM header
   *
   * \return the number of data packets field 
   */
  uint16_t GetNumberOfDataPackets(void) const; //Joao
  /**
   * Return the stream control from the stream control frield of the ATIM header
   *
   * \return the stream control field
   */
  uint16_t GetStreamControl(void) const; //Joao
  uint16_t GetStreamControl2(void) const; //Joao
  /**
...
private:
  ...
  /**
   * Set the Stream Control field of the Atim Header with the given raw value.
   *
   * \param sControl the raw Stream Control field value
   */
  void SetStreamControl (uint16_t sControl); //Joao
  void SetStreamControl2 (uint16_t sControl); //Joao
  /**
  ...
  uint8_t m_channelcode; ///< Channel that this node has chosen for the communication window Joao
  uint8_t m_channelcode_TX; ///< Channel that the transmitter node has chosen for the communication window Joao
  uint16_t m_numberPackets; ///< Number of packets which will be send in the communication window Joao
  uint8_t m_negotiations; //not used
};
