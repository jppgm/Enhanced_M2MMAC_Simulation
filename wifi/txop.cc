...
uint32_t
Txop::GetCw (void) const //no collision window with atimnack
{
  if (m_low->GetAtimNACKCond() == false)
  {
    return m_cw;
  }
  else
...
Txop::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  /*NS_LOG_UNCOND("Queue in txop");
  Mac48Address Kara, Made;
  WifiMacHeader HdrTest;
  Ptr<const Packet> PacketTest;

  Ptr<const WifiMacQueueItem> item = m_queue->Peek ();//hdr.GetAddr2());
  if (item == 0)
  {
    NS_LOG_UNCOND("lalala");
  }
  else 
  {
  PacketTest = item->GetPacket ();
  HdrTest = item->GetHeader ();
  Made = HdrTest.GetAddr1(); //ID do destinatario do pacote
  Kara = HdrTest.GetAddr2(); //ID do remetente do pacote
  NS_LOG_UNCOND("de " << Kara << " para " << Made);
  NS_LOG_UNCOND(HdrTest.IsData());
  }*/

  //NS_LOG_UNCOND("START" << Simulator::Now ().GetSeconds ());
  if (m_low->GetComCond() || m_stationManager->GetIsAtimWindow()) //if it is not the atim window, it must wait for the CTS
  {
    StartAccessIfNeeded ();
  }
...
Txop::GotCts (void)
{
  NS_LOG_UNCOND("GOTCTS" << Simulator::Now ().GetSeconds ());
  StartAccessIfNeeded ();
}

void
Txop::PrepareATIMNACK (void)
{
  //NS_LOG_UNCOND("PREPARINGATIMNACK " << Simulator::Now ().GetSeconds ());
...
Txop::RestartAccessIfNeeded (void)
{
  ...
  if ((m_currentPacket != 0
       || !m_queue->IsEmpty ())
      && !IsAccessRequested ()
      && !m_low->IsCfPeriod () && !m_low->NoCommunicationFlow())
    {
      m_channelAccessManager->RequestAccess (this);
    }
}
...
bool
Txop::NeedAtimRetransmission (Ptr<const Packet> packet, const WifiMacHeader &hdr) //Joao
{
  NS_LOG_FUNCTION (this);
  return m_stationManager->NeedAtimRetransmission (hdr.GetAddr1 (), &hdr, packet);
}
...
Txop::NotifyAccessGranted (void)
{
  ...
  m_accessRequested = false;
  if(m_low->GetAtimNACKCond())
  {
    m_low->SendAtimNACK(false); //send ATIMNACK 
    RestartAccessIfNeeded();
    return;
  }
  if(m_low->NoCommunicationFlow()) //no communication link available
  {
    ResetCw ();
    StartBackoffNow (m_rng->GetInteger (0, GetCw ()));
    return;
  ...
  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      m_currentParams.DisableAtim (); //Joao
...
bool
Txop::MissedAtimAck (void) //Joao, when an ATIMACK is missed
{
  //NS_LOG_UNCOND( "Txop::MissedAtimAck");
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("missed cts");
  bool cond = true;
  if (!NeedAtimRetransmission (m_currentPacket, m_currentHdr))
    {
     // NS_LOG_UNCOND("Nao precisa de retransmissao");
      NS_LOG_DEBUG ("Cts Fail");
      m_stationManager->ReportFinalAtimFailed (m_currentHdr.GetAddr1 (), &m_currentHdr);
...
void
Txop::GotAtimAck (void) //Joao
{
  NS_LOG_FUNCTION (this);
  if (!NeedFragmentation ()
      || IsLastFragment ())
    {
      NS_LOG_DEBUG ("got atimack. tx done.");
    ...
    {
      NS_LOG_DEBUG ("got atimack. tx not done, size=" << m_currentPacket->GetSize ());
    }
}

void 
Txop::RemoveAtim (WifiMacHeader hdr) //Joao, remove the ATIM from the MAC queue to avoid double negotiations
{
  ...
  Ptr<const WifiMacQueueItem> item = m_queue->DequeueByAddress (hdr.GetAddr2());

  if (item == 0) //if the packet is not in the queue, it is waiting to be retransmitted. So, just cancel it
  {
...
void
Txop::CancelRetransmission(void) //Joao
{
