...
void
RegularWifiMac::SetAtimAckTimeout (Time atimAckTimeout) //Joao
{
  NS_LOG_FUNCTION (this << atimAckTimeout);
  m_low->SetAtimAckTimeout (atimAckTimeout);
}
...
Time
RegularWifiMac::GetAtimAckTimeout (void) const //Joao
{
  return m_low->GetAtimAckTimeout ();
}
...
bool
RegularWifiMac::GetIsAtimWindow (void) const
{
  return m_isAtimWindow;
}

void
RegularWifiMac::SetIsAtimWindow (bool enable)
{
...
RegularWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RegularWifiMac")
    ...
    .AddAttribute ("IsAtimWindow", //Joao
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&RegularWifiMac::GetIsAtimWindow,
                                        &RegularWifiMac::SetIsAtimWindow),
                   MakeBooleanChecker ())
...
RegularWifiMac::FinishConfigureStandard (WifiPhyStandard standard)
{
  ...
  switch (standard)
    ...
    case WIFI_PHY_STANDARD_80211_5MHZ:
      ...
      cwmax = 1023;
      break;
    case WIFI_PHY_STANDARD_80211b: //Joao
      SetDsssSupported (true);
      if (GetIsAtimWindow()){
        cwmin = 31;
        cwmax = 1023;
      }
      else{ //in the COM window, there is no backoff
        cwmin = 0;
        cwmax = 0;
      }
      break;
