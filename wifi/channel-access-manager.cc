...
ChannelAccessManager::ChannelAccessManager ()
  : m_lastAckTimeoutEnd (MicroSeconds (0)),
    m_lastAtimAckTimeoutEnd (MicroSeconds (0)), //Joao
...
Time
ChannelAccessManager::MostRecent (Time a, Time b, Time c, Time d, Time e, Time f, Time g, Time h) const //Joao
{
  NS_LOG_FUNCTION (this << a << b << c << d << e << f << g << h);
  Time i = MostRecent (a, b);
  Time j = MostRecent (c, d);
  Time k = MostRecent (e, f);
  Time l = MostRecent (g, h);
  Time m = MostRecent (i, j);
  Time n = MostRecent (k, l);
  Time retval = MostRecent (m, n);
  return retval;
}
...
ChannelAccessManager::GetAccessGrantStart (bool ignoreNav) const
{
  ...
  Time atimAckTimeoutAccessStart = m_lastAtimAckTimeoutEnd + m_sifs; //Joao
  ...
  if (ignoreNav)
    {
      accessGrantedStart = MostRecent (rxAccessStart,
                                       ...
                                       atimAckTimeoutAccessStart, //Joao
...
ChannelAccessManager::GetBackoffStartFor (Ptr<Txop> state)
{
  ...
  //MUDANCA PARA COMM WINDOW SEM DIFS.
  //Time mostRecentEvent = MostRecent (state->GetBackoffStart (),
  //                                   GetAccessGrantStart ());
...
void
ChannelAccessManager::NotifySwitchingStartNow (Time duration) //Joao
{
  NS_LOG_FUNCTION (this << duration);
  ...
  if (m_lastAtimAckTimeoutEnd > now) //Joao
    {
      m_lastAtimAckTimeoutEnd = now;
    }
...
void
ChannelAccessManager::NotifyAtimAckTimeoutStartNow (Time duration) //Joao
{
  NS_LOG_FUNCTION (this << duration);
  NS_ASSERT (m_lastAtimAckTimeoutEnd < Simulator::Now ());
  m_lastAtimAckTimeoutEnd = Simulator::Now () + duration;
}

void
ChannelAccessManager::NotifyAtimAckTimeoutResetNow () //Joao
{
  NS_LOG_FUNCTION (this);
  m_lastAtimAckTimeoutEnd = Simulator::Now ();
  DoRestartAccessTimeoutIfNeeded ();
}
