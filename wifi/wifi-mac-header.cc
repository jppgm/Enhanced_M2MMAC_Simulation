...
enum
{
  //Reserved: 0 - 6 not anymore
  SUBTYPE_CTL_ATIMNACK = 4,
  SUBTYPE_CTL_ATIM = 5, //Joao
  SUBTYPE_CTL_ATIMACK = 6,
...
WifiMacHeader::SetType (WifiMacType type, bool resetToDsFromDs)
{
  switch (type)
    ...
    case WIFI_MAC_CTL_ACK:
      ...
      break;
    case WIFI_MAC_CTL_ATIM: //Joao
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_ATIM;
      break;
    case WIFI_MAC_CTL_ATIMACK:
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_ATIMACK;//joao
      break;
    case WIFI_MAC_CTL_ATIMNACK:
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_ATIMNACK;//joao
      break;
...
void
WifiMacHeader::SetChannelCode(uint8_t channel) //Joao
{
  m_channelcode = channel;
}

void
WifiMacHeader::SetChannelCode_TX(uint8_t channel) //Joao
{
  m_channelcode_TX = channel;
}

void
WifiMacHeader::SetNegotiations(uint8_t negotiations) //Joao
{
  m_negotiations = negotiations;
}

void
WifiMacHeader::SetNumberOfDataPackets(uint16_t numberPackets) //Joao
{
  m_numberPackets = numberPackets;
}
  ...
  switch (m_ctrlType)
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        ...
        case SUBTYPE_CTL_ATIM:
          return WIFI_MAC_CTL_ATIM;
        case SUBTYPE_CTL_ATIMACK:
          return WIFI_MAC_CTL_ATIMACK; //Joao
        case SUBTYPE_CTL_ATIMNACK:
          return WIFI_MAC_CTL_ATIMNACK; //Joao
...
bool
WifiMacHeader::IsAtim (void) const
{
  return (GetType () == WIFI_MAC_CTL_ATIM);
}

bool
WifiMacHeader::IsAtimAck (void) const
{
  return (GetType () == WIFI_MAC_CTL_ATIMACK);
}//Joao

bool
WifiMacHeader::IsAtimNAck (void) const
{
  return (GetType () == WIFI_MAC_CTL_ATIMNACK);
}//Joao
...
uint8_t
WifiMacHeader::GetChannelCode(void) const //Joao
{
  return m_channelcode;
}

uint8_t
WifiMacHeader::GetChannelCode_TX(void) const //Joao
{
  return m_channelcode_TX;
}

uint8_t
WifiMacHeader::GetNegotiations(void) const //Joao
{
  return m_negotiations;
}

uint16_t
WifiMacHeader::GetNumberOfDataPackets(void) const //Joao
{
  return m_numberPackets;
}

uint16_t
WifiMacHeader::GetStreamControl (void) const //Joao
{
  return (m_channelcode | (m_channelcode_TX << 8)) & 0xffff; //I initiate with zero the first four bits (control packet ID)
}

uint16_t
WifiMacHeader::GetStreamControl2 (void) const //Joao, not used
{
  return ((m_channelcode & 0x1f) | ((m_channelcode_TX & 0x1f) << 5) | ((m_negotiations & 0x3f) << 10)) & 0xffff; //I initiate with zero the first four bits (control packet ID)
}
...
void
WifiMacHeader::SetSequenceControl (uint16_t seq) //Joao
{
  m_seqFrag = seq & 0x0f;
  m_seqSeq = (seq >> 4) & 0x0fff;
}
void
WifiMacHeader::SetStreamControl (uint16_t sControl) //Joao, this field also has the number of data packets
{//but it is not used because the network is always saturated
  m_channelcode = sControl & 0x00ff; // & with 111111
  m_channelcode_TX = (sControl >> 8) & 0x00ff; 
}

void
WifiMacHeader::SetStreamControl2 (uint16_t sControl) //Joao, not used
{
  m_channelcode = sControl & 0x001f; // & with 111111
  m_channelcode_TX = (sControl >> 5) & 0x001f; 
  m_negotiations = (sControl >> 10) & 0x003f; 
}
  ...
  switch (m_ctrlType)
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        ...
        case SUBTYPE_CTL_CTS:
        case SUBTYPE_CTL_ACK:
        case SUBTYPE_CTL_ATIMACK: //Joao
        case SUBTYPE_CTL_ATIMNACK: //Joao
          size = 2 + 2 + 6;
          break;
...
WifiMacHeader::Print (std::ostream &os) const
{
  ...
  switch (GetType ())
    ...
    case WIFI_MAC_CTL_ATIM: //Joao 
    os << "Duration/ID=" << m_duration << "us"
         << ", RA=" << m_addr1 << ", TA=" << m_addr2 << ", ChannelCode= " << m_channelcode
          << ", NumberofDataPackets= " << m_numberPackets;
      break;
    case WIFI_MAC_CTL_ATIMACK: //Joao 
      os << "Duration/ID=" << m_duration << "us"
         << ", RA=" << m_addr1 << ", ChannelCode= " << m_channelcode
          << ", NumberofDataPackets= " << m_numberPackets;
      break;
    case WIFI_MAC_CTL_CTS:
    case WIFI_MAC_CTL_ACK:
...
void
WifiMacHeader::Serialize (Buffer::Iterator i) const //Modified by joao, instead of putting the duration field, in the Atim
{                                                    // and atim ack we put the stream control field
  i.WriteHtolsbU16 (GetFrameControl ());
  //i.WriteHtolsbU16 (m_duration);
  if (m_ctrlSubtype == SUBTYPE_CTL_ATIM || m_ctrlSubtype == SUBTYPE_CTL_ATIMACK || m_ctrlSubtype == SUBTYPE_CTL_ATIMNACK)
  {
    i.WriteHtolsbU16 (GetStreamControl());
  }
  else
  {
    i.WriteHtolsbU16 (m_duration);
...
WifiMacHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (m_ctrlSubtype == SUBTYPE_CTL_ATIM || m_ctrlSubtype == SUBTYPE_CTL_ATIMACK || m_ctrlSubtype == SUBTYPE_CTL_ATIMNACK)
  {
    uint16_t stream_control = i.ReadLsbtohU16 ();
    SetStreamControl(stream_control);
  }
  else
  {
    m_duration = i.ReadLsbtohU16 ();
