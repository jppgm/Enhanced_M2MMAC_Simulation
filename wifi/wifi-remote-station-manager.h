...
public:
   ...
   */
  virtual void SetupMac (const Ptr<WifiMac> mac); //Joao
  /**
   * Sets ATIM-Window case.
   *
   * \param m_isAtimWindow
   */
  void SetIsAtimWindow (bool m_isAtimWindow); //Joao
  /**
   * Return if it is ATIM-Window.
   * 
   * \return if it as ATIM-Window
   */
  bool GetIsAtimWindow (void) const;
  /**
   ...
   */
  uint32_t GetMaxSsrc (void) const; //Joao
  /**
  ...
  /**
   * Should be invoked whenever the AtimTimeout associated to a transmission 
   * attempt expires.
   *
   * \param address the address of the receiver
   * \param header MAC header of the DATA packet
   */
  void ReportAtimFailed (Mac48Address address, const WifiMacHeader *header); //Joao
  /**
  ...
  /**
   * Should be invoked whenever we receive the AtimAck associated to an Atim
   * we just sent. Note that we also get the SNR of the Atim we sent since
   * the receiver put a SnrTag in the AtimAck.
   *
   * \param address the address of the receiver
   * \param header MAC header of the DATA packet
   * \param AtimAckSnr the SNR of the AtimAck we received
   * \param AtimAckMode the WifiMode the receiver used to send the AtimAck
   * \param AtimSnr the SNR of the Atim we sent
   */
  void ReportAtimOk (Mac48Address address, const WifiMacHeader *header,
  ...
  /**
   * Should be invoked after calling ReportAtimFailed if
   * NeedRetransmission returns false
   *
   * \param address the address of the receiver
   * \param header MAC header of the DATA packet
   */
  void ReportFinalAtimFailed (Mac48Address address, const WifiMacHeader *header); //Joao
  /**
   ...
   * \param txVector the TXVECTOR of the packet to send
   *
   * \return true if we want to use an Atim/Atimack handshake for this
   *         packet before sending it, false otherwise.
   */
  bool NeedAtim (Mac48Address address, const WifiMacHeader *header, //Joao
                Ptr<const Packet> packet, WifiTxVector txVector);
  /**
   ...
   * \param packet the packet to send
   *
   * \return true if we want to restart a failed ATIM/ATIMACK handshake,
   *         false otherwise.
   */
  bool NeedAtimRetransmission (Mac48Address address, const WifiMacHeader *header, //Joao
                                                 Ptr<const Packet> packet);
  /**
   * \param address remote address
   * \param header MAC header
...
private:
   ...
   *        request that the rts is sent or not.
   *
   * \return true if we want to use an Atim/Atimack handshake for this packet before sending it,
   *         false otherwise.
   *
   * Note: This method is called before a unicast packet is sent on the medium.
   */
  virtual bool DoNeedAtim (WifiRemoteStation *station,
                          Ptr<const Packet> packet, bool normally); //Joao
  /**
   * \param station the station that we need to communicate
   * \param packet the packet to send
   ...
   * \param station the station that we failed to send RTS
   */
   void DoReportAtimFailed (WifiRemoteStation *station); //Joao
  /**
   * This method is a pure virtual method that must be implemented by the sub-class.
   * This allows different types of WifiRemoteStationManager to respond differently,
   *
  ...
  /**
   *
   * \param station the station that we successfully sent Atim
   * \param AtimAckSnr the SNR of the AtimAck we received
   * \param AtimAckMode the WifiMode the receiver used to send the AckAtim
   * \param AtimSnr the SNR of the Atim we sent
   */
  void DoReportAtimOk (WifiRemoteStation *station,
                              double AtimAckSnr, WifiMode AtimAckMode, double AtimSnr); //Joao
  /**
   * This method is a pure virtual method that must be implemented by the sub-class.
   * This allows different types of WifiRemoteStationManager to respond differently,
  ...
  /**
   *
   * \param station the station that we failed to send Atim
   */
  void DoReportFinalAtimFailed (WifiRemoteStation *station); //Joao
  /**
   * This method is a pure virtual method that must be implemented by the sub-class.
   * This allows different types of WifiRemoteStationManager to respond differently,
   *
  ...
  bool m_isAtimWindow;  //If it is ATIM-Window Joao
  ...
  /**
   * The trace source fired when the transmission of a single Atim has failed
   */
  TracedCallback<Mac48Address> m_macTxAtimFailed; //Joao
  /**
  ...
  /**
   * The trace source fired when the transmission of a Atim has
   * exceeded the maximum number of attempts
   */
  TracedCallback<Mac48Address> m_macTxFinalAtimFailed; //Joao
  /**
