...
public:
  ...
  /**
   * Set up constant txop associated with this MacLow.
   *
   * \param txop Txop associated with this MacLow
   */
  void SetTxOp(Ptr<Txop> txop); //Joao
  /**
  ...
  /**
   * Set ATIMACK timeout of this MacLow.
   *
   * \param atimAckTimeout ATIMACK timeout of this MacLow
   */
  void SetAtimAckTimeout (Time atimAckTimeout);
  /**
  ...
  /**
   * Return ATIMACK timeout of this MacLow.
   *
   * \return ATIMACK timeout
   */
  Time GetAtimAckTimeout (void) const;
  bool GetComCond (void) const;
  bool GetAtimNACKCond (void) const;
  /**
  ...
  virtual void StartTransmission (Ptr<const Packet> packet,
                                  ...
                                  Ptr<Txop> txop);
  void SendAtimNACK (bool isATIMACK);
  /**
  ...
  uint8_t SearchChannel(void);
  void UpdateChannel(void);
  void StartATIMNACK(void);
  bool NoCommunicationFlow(void);
  /**
   * \param packet packet received.
   * \param rxSnr snr of packet received.
   *
  ...
  //Ptr<UniformRandomVariable> m_channelrandom; //!< Provides uniform random variables.
...
private:
  ...
  /**
   * Return a TXVECTOR for the ATIM frame given the destination.
   * The function consults WifiRemoteStationManager, which controls the rate
   * to different destinations. It uses the WifiRemoteStationManager->GetRtsTxVector function.
   *
   * \param packet the packet being asked for ATIM TXVECTOR
   * \param hdr the WifiMacHeader
   * \return TXVECTOR for the ATIM of the given packet
   */
  WifiTxVector GetAtimTxVector (Ptr<const Packet> packet, const WifiMacHeader *hdr) const; //Joao
  /**
  ...
  /**
   * Return a TXVECTOR for the ATIMACK frame given the destination and the mode of the ATIM
   * used by the sender.
   * The function consults WifiRemoteStationManager, which controls the rate
   * to different destinations.
   *
   * \param to the MAC address of the ATIMACK receiver
   * \param atimTxMode the mode of the ATIM used by the sender
   * \return TXVECTOR for the ATIMACK
   */
  WifiTxVector GetAtimAckTxVector (Mac48Address to, WifiMode atimTxMode) const; //Joao
  /**
   ...
   * \return TXVECTOR for the ATIMACK
   */
  WifiTxVector GetAtimAckTxVectorForAtim (Mac48Address to, WifiMode atimTxMode) const; //Joao
  /**
   * Return a TXVECTOR for the CTS frame given the destination and the mode of the RTS
   * used by the sender.
   * The function consults WifiRemoteStationManager, which controls the rate
  ...
  /**
   * Return the time required to transmit the ATIMACK (including preamble and FCS).
   *
   * \param atimAckTxVector
   * \return the time required to transmit the atim (including preamble and FCS)
   */
  Time GetAtimAckDuration (WifiTxVector atimAckTxVector) const; //Joao
  /**
   * Return the time required to transmit the ATIMACK to the specified address
   * given the TXVECTOR of the ATIM (including preamble and FCS).
   *
   * \param to
   * \param atimTxVector
   * \return the time required to transmit the ATIMACK (including preamble and FCS)
   */
  Time GetAtimAckDuration (Mac48Address to, WifiTxVector atimTxVector) const; //Joao
  /**
  ...
  /**
   * Check if the current packet should be sent with an ATIM schema.
   *
   * \return true if ATIM should be used,
   *         false otherwise
   */
  bool NeedAtim (void); //Joao
  /**
   ...
   * \param hdr the header
   */
  void NotifyNav (Ptr<const Packet> packet, WifiMacHeader &hdr);
  /**
  ...
  /**
   * Notify ChannelAccessManager that AtimAck timer should be started for the given duration.
   *
   * \param duration the duration
   */
  void NotifyAtimAckTimeoutStartNow (Time duration);
  /**
   * Notify ChannelAccessManager that AtimAck timer should be reset.
   */
  void NotifyAtimAckTimeoutResetNow (); //Joao
  /**
  ...
  /**
   * Event handler when ATIMACK timeout occurs.
   */
  void AtimAckTimeout (void);
  /**
  ...
  /**
   * Send ATIMACK after receiving ATIM.
   *
   * \param source
   * \param duration
   * \param atimTxVector
   * \param atimSnr
   */
  void SendAtimAckAfterAtim (Mac48Address source, WifiTxVector atimTxVector, double atimSnr, uint8_t channelcode_tx); //Joao
  /**
  ...
  /**
   * Send ATIM to begin ATIM-ATIMACK transaction.
   */
  void SendAtimForPacket (void);
  /**
   * Send RTS to begin RTS-CTS-DATA-ACK transaction. //Joao
   */
  ...
  EventId m_atimAckTimeoutEvent;        //!< ATIMACK timeout event // Joao
  EventId m_sendAtimAckEvent;           //!< Event to send ATIMACK //Joao
  ...
  Ptr<Txop> m_constTxop;                    //!< Constant TXOP //Joao
  ...
  Time m_atimAckTimeout;                    //!< ATIMACK timeout duration 
  ...
  bool m_channelCode_channels[26] = {-1};            //!< Number of links Joao
  CfAckInfo m_cfAckInfo; //!< Info about piggyback ACKs used in PCF
  bool m_atimnack_cond;
  bool m_comcond;
  uint8_t last_channel;
  uint8_t nodes_array[61] = {0};
  bool m_retrans_cond;
  bool m_waitingATIMACK;
};
